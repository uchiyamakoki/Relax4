...
import relax.sn.com.relax4.action.CallAction;
import relax.sn.com.relax4.action.CallView;
import relax.sn.com.relax4.action.MessageView;
import relax.sn.com.relax4.action.OpenAppAction;
import relax.sn.com.relax4.action.OpenQA;
import relax.sn.com.relax4.action.ScheduleCreate;
import relax.sn.com.relax4.action.ScheduleView;
import relax.sn.com.relax4.action.SearchAction;
import relax.sn.com.relax4.action.SearchApp;
import relax.sn.com.relax4.action.SearchWeather;
import relax.sn.com.relax4.action.SendMessage;
import relax.sn.com.relax4.entity.AnswerBean;
import relax.sn.com.relax4.entity.DataBean;
import relax.sn.com.relax4.entity.DatetimeBean;
import relax.sn.com.relax4.entity.MainBean;
import relax.sn.com.relax4.entity.ResultBean2;
import relax.sn.com.relax4.entity.SlotsBean;
import relax.sn.com.relax4.utils.JsonParser;
import relax.sn.com.relax4.widget.FiveLine;
import relax.sn.com.relax4.widget.HeartProgressBar;
...
public class VoiceActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // 初始化对象
        mSpeechUnderstander = SpeechUnderstander.createUnderstander(VoiceActivity.this, mSpeechUdrInitListener);

        mToast = Toast.makeText(VoiceActivity.this, "", Toast.LENGTH_SHORT);
    ...
    private void initLayout() {
        ...
        findViewById(R.id.start_understander).setOnClickListener(VoiceActivity.this);
    }
    ...
    public void help(View view) {
        startActivity(new Intent(VoiceActivity.this, HelpActivity.class));
    }
    ...
    public void speakAnswer(String text) {
        // 移动数据分析，收集开始合成事件
        FlowerCollector.onEvent(VoiceActivity.this, "tts_play");
    ...
    private void judgeService() {
        ...
        ResultBean2 resultBean = new ResultBean2();
            ...
            switch (service) {
                ...
                case "telephone":
                    switch (operation) {
                        case "CALL": {    //1打电话
                            ...
                            //可由多个可选条件确定必要条件
                            CallAction callAction = new CallAction(slotsBean.getName(), slotsBean.getCode(), VoiceActivity.this);//目前可根据名字或电话号码拨打电话
                            callAction.start();
                            break;
                        }
                        case "VIEW": {    //2查看电话拨打记录
                ...
                case "message": {//2 短信相关服务
                    ...
                    switch (operation) {
                        ...
                        case "SEND": {//1发送短信
                            SendMessage sendMessage = new SendMessage(slotsBean.getName(), slotsBean.getCode(), slotsBean.getContent(), VoiceActivity.this);
                            sendMessage.start();
                            break;
                        }
                ...
                case "app": {//3 应用相关服务
                    ...
                    switch (operation) {
                        ...
                        case "LAUNCH": {//1打开应用
                            OpenAppAction openApp = new OpenAppAction(slotsBean.getName(), VoiceActivity.this);
                            openApp.start();
                            break;
                        }
                ...
                case "websearch": {//5 搜索相关服务
                    ...
                    switch (operation) {
                        ...
                        case "QUERY": {//1搜索
                            ...
                            SearchAction searchAction = new SearchAction(slotsBean.getKeywords(), VoiceActivity.this);
                            searchAction.Search();
                            break;
                        }
    ...
    protected void onResume() {
        //移动数据统计分析
        FlowerCollector.onResume(VoiceActivity.this);
    ...
    protected void onPause() {
        ...
        FlowerCollector.onPause(VoiceActivity.this);
        super.onPause();
    }
}
